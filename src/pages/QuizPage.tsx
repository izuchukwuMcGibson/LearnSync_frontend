...
const JUDGE0_API_URL =
  ...
  "/judge0-api/submissions?base64_encoded=false&wait=true";

const getJudge0LanguageId = (language: string) => {
  const normalized = language.trim().toLowerCase();
  const languageIds: Record<string, number> = {
    // Judge0 CE language IDs used by the RapidAPI Judge0 endpoint.
    javascript: 63,
    js: 63,
    node: 63,
    nodejs: 63,
  ...
  return languageIds[normalized] || languageIds.javascript;
};

const getExecutionErrorMessage = (data: Judge0SubmissionResponse) => {
  const message = data.message || data.compile_output || data.stderr;
...
const QuizPage = () => {
  ...
  const handleRunCode = async () => {
    ...
    // try {
    //   const apiResponse = await fetch(JUDGE0_API_URL, {
    //     method: "POST",
    //     headers: { "Content-Type": "application/json" },
    //     body: JSON.stringify({
    //       source_code: userPayloadCode,
    ...
    try {
      ...
      const result = (await apiResponse.json()) as Judge0SubmissionResponse;
      ...
      if (
        ...
        setCodeResults((prev) => ({
          ...
          [currentIndex]: {
            success: false,
            output: compileOutput || stderr || status || "Execution failed.",
          },
        }));
        return;
      }
      ...
      const output =
        err.message === "Failed to fetch"
          ? "Unable to reach the code execution service. Confirm the Vite Judge0 proxy is running and RAPID_API_KEY is set in .env."
          : err.message ||
